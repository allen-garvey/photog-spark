...
object ImageController{
    fun show(request: Request, response: Response, imageIdParameterName: String): String {
        val image: Image = SqliteController.selectImage(request.params(imageIdParameterName)) ?:  return ErrorController.notFoundPage(request, response)
    ...
    fun showAlbumImage(request: Request, response: Response, albumIdParameterName: String, imageIdParameterName: String): String {
        ...
        if(imageIndex < 0){
            return ErrorController.notFoundPage(request, response)
        }
