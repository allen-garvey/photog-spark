...
object AlbumController{
    ...
    fun index(request: Request, response: Response): String {
        return AlbumTemplate.index(SqliteController.selectAllAlbums())
    }
    ...
    fun show(request: Request, response: Response, albumIdParameterName: String): String {
        ...
        return SharedTemplate.imageListPage(album.name, SqliteController.imagesForAlbum(request.params(albumIdParameterName)), { AlbumView.urlForAlbumImage(album, it) })
    }
}